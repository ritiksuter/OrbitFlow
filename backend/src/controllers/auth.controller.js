...
export const registerUser = async (req, res) => {
  try {
    ...
    console.log("Arcjet decision", decision.isDenied()); // Give true or false.

    if (decision.isDenied()) {
      res.writeHead(403, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ message: "Invalid email address, Please try again !!" }));
    }
    ...
    if (!isEmailSent) {
      return res.status(500).json({ message: "Failed to send verification email, Something went wrong..." });
    }
